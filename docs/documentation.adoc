...
SELECT (SELECT ROUND((CAST(SUM(uq.timesDONE) AS real) / 193 * 100),2) FROM user_questions uq WHERE uq.q_id = q.q_id) as percent, t.topics AS topics FROM topics.t INNER JOIN subtopics s USING(t_id) INNER JOIN questions q USING(s_id) GROUP BY topics;